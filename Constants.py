...
SPELL_EFFECT_MULTIPLIER = 2.3
LEFTOVER_MULTIPLIER = .5 #Percent of leftover spending from effect generation that is effective for stats
...
EFFECT_COST_DICT = {DRAW_EFFECT:-1.5, DEAL_EFFECT:1, HEAL_EFFECT:-1, SUMMON_EFFECT:-1.5, BUFF_EFFECT:-2, SPLIT_DEAL_EFFECT:1.2, SPLIT_HEAL_EFFECT:-1.2}#Split Damage and heal are so high because they only work with random targets which have low values, this offsets them a bit #This converts the identity of the effect to the cost of it when generating effects
...
TARGET_COST_DICT = {TARGET_BOTH:.6, TARGET_ALL:1.5, TARGET_OWN_PLAYER:-1, TARGET_CREATURE:1.2, TARGET_OPPONENT:1, TARGET_PLAYERS:1.1, TARGET_RANDOM:.6, TARGET_RANDOM_ENEMY:.8, TARGET_RANDOM_ALLY:-.7, TARGET_RANDOM_CREATURE:.6, TARGET_RANDOM_ENEMY_CREATURE:.8, TARGET_RANDOM_ALLY_CREATURE:-.7, TARGET_ALL_CREATURE:1.5, TARGET_THIS_CREATURE:-1}
